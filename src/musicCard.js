...
function Header({ isHidden }) {
  return (
    <header className={`header retro-header ${isHidden ? "hidden" : ""}`}>
      <h1 className="header-title transition-text">Хроники Саундклауда</h1>
      <p className="header-description transition-text">
        Погружение в атмосферу независимой музыки и андеграундных битов.
      </p>
    </header>
  );
}
...
export default function MusicPortfolio() {
  const [currentIndex, setCurrentIndex] = useState(0); // 0 - заголовок, 1+ - карточки
  const [isTransitioning, setIsTransitioning] = useState(false);

  const changeTrack = (newIndex) => {
    if (isTransitioning) return; // Блокируем спам кнопок

    setIsTransitioning(true);
    setTimeout(() => {
      setCurrentIndex(newIndex);
      setTimeout(() => setIsTransitioning(false), 500); // Ждём завершения анимации
    }, 300);
  };
